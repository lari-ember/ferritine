...
class AgentStatus(str, enum.Enum):
    ...
    QUEUED_FOR_RESOURCE = (
        "queued_for_resource"  # Em fila por recurso (p.ex. energia/água)
    )
...
class Agent(Base):
    ...
    current_location_type = Column(
        String(50), nullable=True, comment="building, vehicle, street, etc"
    )
    ...
    __table_args__ = (
        CheckConstraint(
            "energy_level >= -100 AND energy_level <= 100", name="check_energy_range"
        ),
    )

    # Humor/Emoção/Sentimento (polimórfico, complexo como The Sims 4)
    mood_data = Column(
        JSON, default=dict, comment="Sistema complexo de humor que afeta comportamento"
    )
    ...
    # Versão do programa
    version = Column(
        String(20), nullable=False, comment="Versão do programa quando criado/nasceu"
    )
    ...
    # Genética complexa
    genetics = Column(
        JSON, default=dict, comment="Sistema de genética complexa herdada dos pais"
    )

    # Relacionamentos
    routine = relationship(
        "Routine", back_populates="agents", foreign_keys=[routine_id]
    )
    profession = relationship("Profession", back_populates="agents")

    # Edifícios
    home = relationship(
        "Building", foreign_keys=[home_building_id], back_populates="residents"
    )
    workplace = relationship(
        "Building", foreign_keys=[work_building_id], back_populates="workers"
    )
    owned_buildings = relationship(
        "Building", foreign_keys="Building.owner_id", back_populates="owner"
    )
...
class Routine(Base):
    ...
    schedule = Column(JSON, default=list, comment="Programação por hora do dia")
...
class Profession(Base):
    ...
    # Tipo de trabalho
    work_sector = Column(
        String(50), comment="residential, commercial, industrial, public, etc"
    )
...
class BuildingType(str, enum.Enum):
    ...
    INDUSTRIAL_CONSTRUCTION_YARD = (
        "industrial_construction_yard"  # Depósito de construção
    )
    ...
    TRANSPORT_TRAIN_STATION_CENTRAL = (
        "transport_train_station_central"  # Estação central
    )
...
class BuildingStatus(str, enum.Enum):
    ...
    OPERATIONAL_CLOSED_TEMPORARY = (
        "operational_closed_temporary"  # Fechado temporariamente
    )
...
class Building(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "condition_value >= 0 AND condition_value <= 100",
            name="check_building_condition_range",
        ),
    )

    # ==================== PROPRIEDADE ====================
    owner_id = Column(
        GUID(), ForeignKey("agents.id"), nullable=True, comment="ID do agente dono"
    )
    owner_type = Column(
        SQLEnum(BuildingOwnershipType), default=BuildingOwnershipType.PRIVATE_INDIVIDUAL
    )

    # ==================== ARQUITETURA ====================
    architecture_style = Column(
        SQLEnum(BuildingArchitectureStyle), default=BuildingArchitectureStyle.GENERIC
    )
    ...
    foundation_type = Column(
        String(50), default="concrete", comment="concreto, madeira, pedra"
    )
    structure_type = Column(
        String(50), default="brick", comment="tijolo, concreto, aço, madeira"
    )
    ...
    utility_costs = Column(
        DECIMAL(13, 2), default=0.00, comment="Água, luz, gás (mensal)"
    )
    ...
    rental_income = Column(
        DECIMAL(13, 2), default=0.00, comment="Aluguel recebido (mensal)"
    )
    business_revenue = Column(
        DECIMAL(13, 2), default=0.00, comment="Receita de negócio (mensal)"
    )
    ...
    major_events = Column(
        JSON, default=list, comment="Eventos importantes (incêndios, reformas, etc)"
    )
    ...
    # ==================== GAMEPLAY ====================
    happiness_modifier = Column(
        Float, default=0.0, comment="Bônus/penalidade para moradores"
    )
    ...
    led_pin = Column(
        Integer, nullable=True, comment="Pino do Arduino para controlar LED"
    )
    ...
    owner = relationship(
        "Agent", foreign_keys=[owner_id], back_populates="owned_buildings"
    )

    # Moradores (agentes que moram aqui)
    residents = relationship(
        "Agent", foreign_keys="Agent.home_building_id", back_populates="home"
    )

    # Trabalhadores (agentes que trabalham aqui)
    workers = relationship(
        "Agent", foreign_keys="Agent.work_building_id", back_populates="workplace"
    )
    ...
    def calculate_monthly_income(self) -> float:
        ...
        return float(self.rental_income + self.business_revenue)
    ...
    def is_operational(self) -> bool:
        ...
        return (
            self.status == BuildingStatus.OPERATIONAL_ACTIVE
            and self.condition_value > 20
        )
...
class Station(Base):
    ...
    building_id = Column(
        ...
        nullable=True,
        comment="Edifício associado (opcional)",
    )
    name = Column(String(200), nullable=False, index=True)
    ...
    max_vehicles_docked = Column(
        Integer, default=2, comment="Máximo de veículos simultaneamente"
    )
    ...
    # ==================== RELACIONAMENTOS ====================
    building = relationship("Building", foreign_keys=[building_id], backref="stations")
...
class Vehicle(Base):
    ...
    def __repr__(self):
        return (
            f"<Vehicle(id={self.id}, name='{self.name}', type='{self.vehicle_type}')>"
        )
...
class NamePool(Base):
    ...
    gender = Column(
        SQLEnum(Gender), nullable=True, comment="Alguns nomes são específicos de gênero"
    )

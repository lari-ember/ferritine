...
from sqlalchemy import (
    ...
    JSON, CheckConstraint, Index, TypeDecorator, Time, LargeBinary
)
...
class Stop(Base):
    ...
    max_queue_length = Column(Integer, default=100, comment="Capacidade máxima de fila")
...
#todo Adicionar relacionamento reverso em Building (adicionar  classe Building existente): stations = relationship('Station', foreign_keys='Station.building_id', back_populates='building')
...
class VoxelChunk(Base):
    """
    Chunk de voxels para sistema de terreno voxelizado.
    
    Armazena dados volumétricos comprimidos (64x64x64 voxels por chunk).
    Usado para renderização eficiente com Unity DOTS.
    
    Escala:
    - 1 voxel = 3.6cm
    - 1 chunk = 64³ voxels = ~2.3m x 2.3m x 2.3m
    """
    __tablename__ = 'voxel_chunks'
    
    id = Column(GUID(), primary_key=True, default=uuid.uuid4)
    
    # Coordenadas do chunk (não do voxel individual)
    chunk_x = Column(Integer, nullable=False, index=True)
    chunk_y = Column(Integer, nullable=False, index=True)
    chunk_z = Column(Integer, nullable=False, index=True, comment="Altura base do chunk")
    
    # Dados comprimidos (zlib)
    # Header (16 bytes) + voxel data comprimido
    data_compressed = Column(LargeBinary, nullable=False, comment="Dados do chunk comprimidos com zlib")
    
    # Metadados para otimização
    data_hash = Column(String(32), nullable=True, comment="MD5 hash para validação de cache")
    solid_count = Column(Integer, default=0, comment="Número de voxels sólidos (não-ar)")
    is_empty = Column(Boolean, default=False, comment="True se chunk contém apenas ar")
    
    # Versionamento para invalidação de cache
    version = Column(Integer, default=1, comment="Incrementa quando chunk é modificado")
    
    # LOD (Level of Detail) pré-computado
    lod1_data = Column(LargeBinary, nullable=True, comment="LOD1: 32³ voxels comprimido")
    lod2_data = Column(LargeBinary, nullable=True, comment="LOD2: 16³ voxels comprimido")
    
    # Timestamps
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    
    # Índices compostos para queries espaciais eficientes
    __table_args__ = (
        Index('ix_voxel_chunk_coords', 'chunk_x', 'chunk_y', 'chunk_z', unique=True),
        Index('ix_voxel_chunk_xy', 'chunk_x', 'chunk_y'),  # Para queries de área
        CheckConstraint('solid_count >= 0', name='check_voxel_solid_count_positive'),
    )
    
    def __repr__(self):
        return f"<VoxelChunk(x={self.chunk_x}, y={self.chunk_y}, z={self.chunk_z}, solids={self.solid_count})>"


class VoxelModification(Base):
    """
    Log de modificações em voxels individuais.
    
    Usado para:
    - Histórico de terraformação
    - Sincronização incremental com clientes
    - Undo/redo de operações
    """
    __tablename__ = 'voxel_modifications'
    
    id = Column(GUID(), primary_key=True, default=uuid.uuid4)
    
    # Chunk afetado
    chunk_id = Column(GUID(), ForeignKey('voxel_chunks.id'), nullable=False, index=True)
    
    # Posição do voxel dentro do chunk (0-63)
    local_x = Column(Integer, nullable=False)
    local_y = Column(Integer, nullable=False)
    local_z = Column(Integer, nullable=False)
    
    # Mudança
    old_type = Column(Integer, nullable=False, comment="VoxelType anterior")
    new_type = Column(Integer, nullable=False, comment="VoxelType novo")
    
    # Contexto
    modification_type = Column(String(50), default='manual', comment="manual, construction, erosion, etc")
    caused_by_agent_id = Column(GUID(), ForeignKey('agents.id'), nullable=True)
    caused_by_building_id = Column(GUID(), ForeignKey('buildings.id'), nullable=True)
    
    # Timestamps
    created_at = Column(DateTime, default=datetime.utcnow, index=True)
    
    # Relacionamentos
    chunk = relationship('VoxelChunk', backref='modifications')
    
    __table_args__ = (
        CheckConstraint('local_x >= 0 AND local_x < 64', name='check_voxel_mod_local_x'),
        CheckConstraint('local_y >= 0 AND local_y < 64', name='check_voxel_mod_local_y'),
        CheckConstraint('local_z >= 0 AND local_z < 64', name='check_voxel_mod_local_z'),
    )
    
    def __repr__(self):
        return f"<VoxelModification(chunk={self.chunk_id}, pos=({self.local_x},{self.local_y},{self.local_z}), {self.old_type}->{self.new_type})>"
